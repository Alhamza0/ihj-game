...
- **تحكيم أذكى هجين:** عند انتهاء الجولة (`collect`) يتحقّق الخادم أن الإجابات كلمات حقيقية: قاموس offline للبلدان/المدن ([shared/dictionaries.js](shared/dictionaries.js)) + Claude Haiku للبقية ([server/judge.js](server/judge.js)، نداء واحد/جولة + كاش). المرفوض يُعلَّم في `room.scoreInvalid` تلقائياً و«🤖 رُفض آلياً» في الواجهة، والمضيف يعكس بزر التجاوز. مُسوَّر بـ `ANTHROPIC_API_KEY` (بدونه: إلغاء يدوي كالسابق). القاموس لا يرفض أبداً.
